...
受測者跑測試(story)──POST(anon, insert-only)──▶  Supabase: ut_results（含個資）
                                                          ▲
PM / designer 看 dashboard(瀏覽器)                       │ service_role 讀(bypass RLS)
      │                                                   │
      ▼  GET /.netlify/functions/ut-data                  │
  UT Dashboard story ◀──JSON──  Netlify Function ─────────┘
      （整站掛 Basic Auth edge function）      （service_role key 只存 Netlify env,不進前端 / repo）
```

**關鍵安全點**:`ut_results` 含個資(姓名 / email / 逐字稿),而 anon key 是 public(在 repo 裡)。
所以**不開** anon SELECT;改由 server-side function 用 **service_role key** 讀(service_role bypass
RLS,不需改任何 policy)。前端只拿得到 function 回傳的 JSON,且整站掛 Netlify Basic Auth。
...
| Key | Value | 哪裡拿 |
|---|---|---|
| `SUPABASE_URL` | `https://qjaedugymiezllhhtbgs.supabase.co` | Supabase → Project Settings → API → Project URL |
| `SUPABASE_SERVICE_ROLE_KEY` | `eyJ...`(**secret,勿外流 / 勿進 repo**) | Supabase → Project Settings → API → `service_role` |
...
| 檔 | 作用 |
|---|---|
| `netlify/functions/ut-data.mts` | 讀取層(service_role 讀 Supabase + defense-in-depth Basic Auth)|
