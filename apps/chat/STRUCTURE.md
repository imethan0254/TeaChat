...
├── InputBox                  (接受 `fullWidth` + `onSend` prop。左右邊緣對齊 MessageArea chat-bubble region：ON=全寬左右 16px / OFF=max 960px 置中。單行：textarea + buttons 同排；多行：textarea 全寬在上，buttons 獨立在下。外框 padding top/bottom 6px / left 12px / right 8px；整個輸入方塊 max-height 280px（textarea clamp 232）；輸入後外框轉 primary-hover 藍框；Send 按鈕無值時 text variant(無底深線)、有值時 primary。外層 pt-2=8px / pb-4=16px)
