...
> - `Message.repliedToThreadParentId?: string`：標記「主區的 thread 回覆副本」。`MessageBubble`（`!isInThread` 時）偵測到 → 渲染 **repliedLink**：左側 L-connector（`border-l border-b rounded-bl-[8px]` 24×12，neutral-4，**mine/other 皆顯示**）+ `MessagesSquare 16` + 「**replied to a thread:**」prefix（`shrink-0 whitespace-nowrap`，**永不截斷**）+ 母訊息 text（`min-w-0 truncate`，**只截這段**）。全部色 `var(--color-neutral-7)`。prefix shrink-0 → link row 可比 bubble 更寬以完整顯示 prefix（受 column flex-1 / max-w-full 上限約束，超出時母訊息 text 截斷）。點擊 `onOpenThread(parent)` 開該 thread。主區同時渲染 `threadLink`(有 threadMessages 時) 與 `repliedLink`(reply 副本時)，兩者互斥。
